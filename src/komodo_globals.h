...
std::string BITSUM_CHEATCATCHER, NOTARY_PUBKEY,ASSETCHAINS_NOTARIES,ASSETCHAINS_OVERRIDE_PUBKEY,DONATION_PUBKEY;
uint8_t NOTARY_PUBKEY33[33],ASSETCHAINS_OVERRIDE_PUBKEY33[33],ASSETCHAINS_PUBLIC,ASSETCHAINS_PRIVATE;
bool BITSUM_MINTBLOCKS;
...
// Bitsum proof of stake controls
int32_t ASSETCHAINS_LWMAPOS = 0;         // percentage of blocks should be PoS
int32_t BITSUM_BLOCK_POSUNITS = 1024;    // one block is 1000 units
int32_t BITSUM_MIN_STAKEAGE = 150;       // 1/2 this should also be a cap on the POS averaging window, or startup could be too easy
int32_t BITSUM_CONSECUTIVE_POS_THRESHOLD = 7;
int32_t BITSUM_NOPOS_THRESHHOLD = 150;   // if we have no POS blocks in this many blocks, set to default difficulty
